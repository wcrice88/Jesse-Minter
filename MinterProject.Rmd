...
```{r}
plot1 = ggplot(chargers_weekly, aes(x = week, y = chargers_poe, fill = location)) +
...
```{r}
plot2 = ggplot(yard_stats, aes(x = factor(season), y = value, fill = stat)) +
...
```{r}
plot3 = ggplot(def_stats, aes(x = factor(season), y = value, fill = stat)) +
...
```{r}
plot4 = ggplot(lac_heatmap, aes(x = factor(season), y = stat, fill = percentile)) +
...
```{r}
plot5 = ggplot(quadrant_data, aes(x = post_pct, y = pre_pct)) +
...
```{r}
plot7 = ggplot(def_compare, aes(x = epa_21_23, y = epa_24_25)) +
